...
const Grimoire: React.FC<GrimoireProps> = ({ currentPath, onOpenDocument, onOpenProject }) => {
    ...
    // ─── Render Projects ───
    // Unified launcher: folder-based projects and in-app writing projects live in one list.
    const renderProjects = () => {
        ...
        const hasProjects = recentProjects.length > 0 || projects.length > 0;
                ...
                <div className="max-w-4xl mx-auto w-full">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="px-3 py-1.5 text-xs theme-bg-tertiary theme-hover rounded flex items-center gap-1.5"
                            >
                    ...
                    {!hasProjects ? (
                        <div className="flex flex-col items-center justify-center py-16 text-gray-500">
                            ...
                            <p className="text-xs text-gray-500 mb-6">Open a folder or create a new writing project.</p>
                        ...
                        <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4">
                            {recentProjects.map(p => (
                                <div key={p.path}
                                    ...
                                    <div className="h-48 bg-gradient-to-b from-slate-700 to-slate-950 p-4 flex flex-col justify-end">
                                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={(e) => { e.stopPropagation(); handleRemoveRecentProject(p.path); }}
                                                className="p-1.5 bg-red-600/80 rounded hover:bg-red-600" title="Remove">
                                                <X size={12} />
                                            </button>
                                        </div>
                                        <div className="text-[10px] uppercase tracking-wide text-white/60 mb-1">Folder</div>
                                        <h3 className="font-bold text-white text-lg leading-tight truncate">{p.name}</h3>
                                        <p className="text-xs text-white/70 mt-1 truncate">{p.path}</p>
                                    </div>
                                    <div className="p-3 theme-bg-secondary">
                                        <div className="flex items-center justify-between text-[10px] text-gray-400">
                                            <span>LaTeX / files</span>
                                            <span>Opened {new Date(p.openedAt).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
