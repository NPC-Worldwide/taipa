...
function getBackendScriptPath(): string | null {
  ...
  const archivedPath = path.join(__dirname, '..', 'resources', 'taipa_serve.py');
  ...
  try {
    ...
    const extractedPath = path.join(extractedDir, 'taipa_serve.py');
...
async function startBackend() {
  ...
  if (!scriptPath) {
    console.error('[Main] Backend script taipa_serve.py not available');
    return false;
  }
...
const UPDATE_MANIFEST_URL = 'https://storage.googleapis.com/taipa-executables/manifest.json';
...
ipcMain.handle('download-and-install-update', async (event, { releaseUrl }: { releaseUrl: string }) => {
  try {
    const tmpDir = path.join(os.tmpdir(), 'taipa-update');
    await fsPromises.mkdir(tmpDir, { recursive: true });
    const fileName = path.basename(new URL(releaseUrl).pathname) || 'taipa-update';
